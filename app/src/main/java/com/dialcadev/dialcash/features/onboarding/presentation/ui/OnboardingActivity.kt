package com.dialcadev.dialcash.features.onboarding.presentation.ui
...
import com.dialcadev.dialcash.features.onboarding.presentation.adapter.OnboardingAdapter
import com.dialcadev.dialcash.features.onboarding.presentation.model.OnboardingItem
import com.dialcadev.dialcash.features.onboarding.presentation.viewmodels.OnboardingViewModel
...
class OnboardingActivity : AppCompatActivity() {
    ...
    private fun setupViews() {
        ...
        pager.registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                updateDots(position)
                binding.btnNext.text = if (position == items.size - 1) getString(R.string.finish) else getString(R.string.next)
            }
        })
        binding.btnNext.setOnClickListener {
            if (pager.currentItem < items.size - 1) pager.currentItem += 1
            else {
                vm.markSeen()
                finishOnboarding()
            }
        ...
        vm.isSeen.let { _ ->
            lifecycleScope.launch {
                lifecycle.repeatOnLifecycle(Lifecycle.State.STARTED) {
                    vm.isSeen.collect { isSeen ->
                        if (isSeen) finishOnboarding()
                    }
                }
            }
        }
    ...
    private fun setupDotsIndicator(count: Int) {
        ...
        repeat(count) {
            val dots = ShapeableImageView(this).apply {
    ...
    private fun updateDots(position: Int) {
        ...
        for (i in 0 until container.childCount) {
            val dot = container.getChildAt(i) as ShapeableImageView
            val drawableRes = if (i == position) R.drawable.indicator_active else R.drawable.indicator_inactive
