...
fun Context.showTransactionDetailsBottomSheet(
    ...
    val binding = RecycleTransactionItemBinding.inflate(LayoutInflater.from(this))
    ...
    fun validateForm(): Boolean {
        binding.apply {
            ...
            if (amountText.isEmpty()) {
                tilTransactionAmount.error = getString(R.string.amount_required)
                isValid = false
                ...
                isValid = false
            } else tilTransactionAmount.error = null

            if (descriptionText.isEmpty()) {
                tilTransactionDescription.error =
                    root.context.getString(R.string.description_cannot_be_empty)
                isValid = false
            } else tilTransactionDescription.error = null
            ...
            if (transaction.type == "transfer") {
                ...
                if (toAccountText.isEmpty() || accounts.none { it.name == toAccountText }) {
                    ...
                    tilAccountToName.error =
                        root.context.getString(R.string.cannot_transfer_same_acc)
                    isValid = false
                } else tilAccountToName.error = null
            }
            if (transaction.type == "expense") {
                val groupText = actvIncomeGroupName.text.toString().trim()
                if (groupText.isNotEmpty() && incomeGroups.none { it.name == groupText }) {
                    tilIncomeGroupName.error =
                        root.context.getString(R.string.select_valid_income_group)
                    isValid = false
                } else tilIncomeGroupName.error = null
                ...
                onValidateBalance(
                    ...
                    if (valid) {
                        ...
                        when (transaction.type) {
                            ...
                            "transfer" -> tilAccountName.error = msg
                        }
                        btnSave.isEnabled = false
                    }
                }
    ...
    binding.apply {
        ...
        ivTransactionType.setColorFilter(getColor(color))
        tvTransactionAmount.text = amountText
        tvTransactionAmount.setTextColor(getColor(color))
        ...
        if (transaction.type == "transfer" && transaction.accountToName != null) {
            layoutTransferTo.visibility = View.VISIBLE
        }
        tvAccountToName.text = transaction.accountToName ?: "N/A"

        if (transaction.type == "expense" && transaction.incomeGroupName != null) {
            layoutIncomeGroup.visibility = View.VISIBLE
        }
    ...
    binding.apply {
        ...
        val accountAdapter = ArrayAdapter(
            this@showTransactionDetailsBottomSheet,
    ...
    binding.apply {
        ...
        actvAccountToName.setOnItemClickListener { _, _, position, _ ->
            ...
            selectedAccountTo = accounts.find { it.name == selectedName }
            validateForm()
        }
        actvIncomeGroupName.setOnItemClickListener { _, _, position, _ ->
            ...
            selectedIncomeGroup = incomeGroups.find { it.name == selectedName }
            validateForm()
        }
    ...
    binding.apply {
        ...
        btnCancel.setOnClickListener {
            if (actionsRow.isGone && editFooter.isVisible) resetView()
            else dialog.dismiss()
        }
        ...
        btnCancelDelete.setOnClickListener {
            if (deleteConfirmFooter.isVisible && actionsRow.isGone) resetView()
            else dialog.dismiss()
        }

        btnConfirmDelete.setOnClickListener {
            val txId = transaction.id ?: run { dialog.dismiss(); return@setOnClickListener }
            val accountId = accounts.find { it.name == transaction.accountName }?.id
                ?: run { dialog.dismiss(); return@setOnClickListener }
            val accountToId =
                if (transaction.type == "transfer" && transaction.accountToName != null) {
            ...
            onDelete(
                Transaction(
                    ...
                    type = transaction.type ?: "expense", // o el enum, si lo usas
        ...
        btnSave.setOnClickListener {
            ...
            val newAccountId =
                accounts.find { it.name == selectedAccountName }?.id ?: return@setOnClickListener
